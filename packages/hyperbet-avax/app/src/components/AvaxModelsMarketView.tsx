...
} from "../lib/useMockAvaxStreamData";
...
const DEFAULT_SKEW_SCALE_AVAX = 10;
...
function computeMockMarketSnapshot(entry: MockLeaderboardEntry): MarketSnapshot {
  ...
  const fundingRate = (longOi - shortOi) / (DEFAULT_SKEW_SCALE_AVAX * 100);
  ...
  return {
    ...
    skewScaleAvax: DEFAULT_SKEW_SCALE_AVAX,
  };
}
...
function leaderboardEntryToDirectoryEntry(
  ...
  return {
    characterId: agentNameToCharacterId(entry.agentName),
...
): { entries: PerpsDirectoryEntry[]; updatedAt: number } {
  const raw = Array.isArray(payload.markets) ? payload.markets : [];
  const entries = raw
    .map((m) => sanitizeDirectoryEntry(m))
    .filter((e): e is PerpsDirectoryEntry => e !== null);
  return {
    entries,
...
export function AvaxModelsMarketView({
  ...
  const [positions] = React.useState<Record<string, PositionSnapshot>>({});
  ...
  // ── Trade inputs ──
  const [collateralAvax, setCollateralAvax] = React.useState(0.1);
  ...
  React.useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(`${GAME_API_URL}/api/perps/markets`, {
          cache: "no-store",
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const payload = (await res.json()) as ApiPerpsMarketsResponse;
        if (cancelled) return;
        const { entries: fetched, updatedAt } = sanitizeMarketsResponse(payload);
        setEntries(fetched);
        setDataError(null);
        setDataLoading(false);
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `${GAME_API_URL}/api/perps/oracle-history?characterId=${encodeURIComponent(selectedId)}&limit=${ORACLE_HISTORY_LIMIT}`,
          { cache: "no-store" },
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const skewScale = selectedMarket?.skewScaleAvax ?? DEFAULT_SKEW_SCALE_AVAX;
  const estLongPrice = estimateExecutionPrice(
    selectedMarket,
    collateralAvax * effectiveLeverage,
    skewScale,
  );
  ...
  const handleOpenPosition = async (direction: TradeDirection) => {
    ...
    toast.info(
      "EVM perps contracts are deploying — trading will be enabled soon.",
      { id: txId, duration: 4000 },
    );
  ...
  const handleClosePosition = async () => {
    ...
    toast.info("EVM perps contracts are deploying — position management coming soon.");
  };

  // ── Render ──────────────────────────────────────────────────────────────────

  return (
    <div className="hm-perps-view" data-testid="models-market-view">
      <Toaster theme="dark" position="bottom-right" />

      {/* ── Hero ── */}
      <section className="hm-perps-hero">
        <div className="hm-perps-hero-text">
          <p className="hm-perps-kicker">Synthetic Model Perps · Avalanche C-Chain</p>
            ...
            <>
              ...
              <div className="hm-perps-detail-grid">
                ...
                <div className="hm-perps-detail-item">
                  ...
                  <strong className="hm-perps-detail-value">
                    {selectedMarket
                      ? `${selectedMarket.longOi.toFixed(2)} / ${selectedMarket.shortOi.toFixed(2)} AVAX`
                      : "—"}
                  </strong>
                </div>
                <div className="hm-perps-detail-item">
                ...
                <div className="hm-perps-detail-item">
                  ...
                  <strong className="hm-perps-detail-value">
                    {selectedMarket
                      ? `${selectedMarket.insuranceFund.toFixed(2)} AVAX`
                      : "—"}
                  </strong>
                </div>
              </div>
              ...
              {selectedPosition && (
                <div className="hm-perps-position-card">
                  <div className="hm-perps-position-header">
                    ...
                    >
                      {selectedPosition.direction} {selectedPosition.size.toFixed(3)} AVAX
                    </strong>
                  </div>
                  <div className="hm-perps-position-stats">
                    <span>Entry ${selectedPosition.entryPrice.toFixed(2)}</span>
                    <span>Margin {selectedPosition.margin.toFixed(4)} AVAX</span>
                    <span>
                      PnL{" "}
                      <strong
                        className={
                      ...
                      >
                        {selectedPosition.pnl >= 0 ? "+" : ""}
                        {selectedPosition.pnl.toFixed(4)} AVAX
                      </strong>
                    </span>
              ...
              <div className="hm-perps-trade-card">
                <div className="hm-perps-trade-header">
                  ...
                  <span className="hm-perps-chain-badge">AVAX</span>
                </div>

                {/* Collateral input */}
                <label className="hm-perps-field">
                  <span className="hm-perps-field-label">Collateral (AVAX)</span>
                  <input
                    className="hm-perps-field-input"
                    type="number"
                    min={0.01}
                    step={0.01}
                    value={collateralAvax}
                    onChange={(e) => setCollateralAvax(Number(e.target.value))}
                  />
                </label>
                ...
                <div className="hm-perps-summary">
                  <div className="hm-perps-summary-row">
                    ...
                    <strong>
                      {(collateralAvax * effectiveLeverage).toFixed(3)} AVAX
                    </strong>
                  </div>
                  <div className="hm-perps-summary-row">
                ...
                <div className="hm-perps-actions">
                  <button
                    ...
                    disabled={Boolean(submittingTrade) || collateralAvax <= 0}
