...
LEDGER_DIR="${E2E_SOLANA_LEDGER_DIR:-/tmp/hyperbet-solana-e2e-ledger-${SOLANA_RPC_PORT}}"
...
resolve_wallet_path() {
  ...
  echo "[e2e] no Anchor wallet found for upgradeable local validator" >&2
  exit 1
}
...
if ! wait_for_app "http://127.0.0.1:$APP_PORT/" "$APP_PID" "$APP_PORT"; then
